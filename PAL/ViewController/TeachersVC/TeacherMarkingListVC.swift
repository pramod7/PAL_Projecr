//
//  TeacherMarkingListVC.swift
//  PAL
//
//  Created by i-Verve on 08/01/21.
//

import UIKit
import SDWebImage

class TeacherMarkingListVC: UIViewController,UICollectionViewDelegate, UICollectionViewDataSource, SubjectListDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let nav = self.navigationController{
            ...
            titleLabel.navTitle(strText: "Marking", titleColor: .white)
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return self.arrMarkingList.count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if DeviceType.IS_IPHONE {
            ...
            if indexPath.row % 2 == 0 {
                ...
                cell.lblTeacherName.text = "Teacher : \(self.arrMarkingList[indexPath.row].teacherName ?? "")"
                cell.lblTeacherId.text = "Teacher ID : \(self.arrMarkingList[indexPath.row].teacherId ?? "")"
                cell.lblDate.text = self.arrMarkingList[indexPath.row].assignDate
                cell.imgMarking.imageFromURL( self.arrMarkingList[indexPath.row].pdfThumb!, placeHolder: imgWorksheetPlaceholder)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let isCompleted = self.arrMarkingList[indexPath.row].isCompleted, isCompleted == 0{
            ...
            objNext.studentId = self.childId
            ...
            objNext.arrImages = self.arrMarkingList[indexPath.row].pdfImage ?? []
            ...
            if let arrvoiceInstruction = self.arrMarkingList[indexPath.row].voiceinstruction{
                objNext.arrInstruction = arrvoiceInstruction
            }
            self.navigationController?.pushViewController(objNext, animated: true)
        }
    }
    ...
    func APICallGetMarkingList() {
        ...
        APIManager.shared.callPostApi(parameters: params, reqURL: URLs.APIURL + getUserTye() + getMarkingTeacherList, showLoader: true, vc:self) { (jsonData, error) in
            
            if jsonData != nil {
                if let userData = ObjectResponse(JSON: jsonData!.dictionaryObject!){
                    if let status = userData.status, status == 1{
                        if let linkTeacher = userData.marking {
                            self.arrMarkingList = linkTeacher
                        ...
                        if self.arrMarkingList.count > 0{
                            ...
                            lbl.text = "No marking worksheet(s) found"
